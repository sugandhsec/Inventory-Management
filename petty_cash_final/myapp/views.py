...
def signup(request):
	if request.method=="POST":
		try:
			...
			final_amount=amount_fetch.amount_present-int(request.POST['amount'])
...
def visitor_entry(request,pk):
	if request.method=="POST":
		...
		userdetails.amount=int(userdetails.amount)+int(request.POST['address'])
		userdetails.save()
		amount_fetch=petty_amount.objects.get()
...
def add_amount(request):
	if request.method=="POST":
		amount_fetch=petty_amount.objects.get()
		final_amount=amount_fetch.amount_present+int(request.POST['amount_add'])
		amount_fetch.amount_present=final_amount
		amount_fetch.save()
		user=User.objects.get(email=request.session['email'])
		amount_now=petty_amount.objects.get()
...
def return_amount(request,pk):
	if request.method=="POST":
		...
		final_amount=int(amount_details.amount)-int(request.POST['amount_add'])
